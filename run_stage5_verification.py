...
def main():
    ...
    rc2 = run_cmd(
        ...
        f'"{sys.executable}" -m pytest personalized_precision_oncology/stage5_genai/data_engineering/tests -v'
    )
    ...
    rc3 = run_cmd(
        ...
        f'"{sys.executable}" -m pytest personalized_precision_oncology/stage5_genai/eda_prompteng/tests -v'
    )

    # 4. Run GenAI Scenario Generator Tests
    print_banner("4. Running Stage 5 GenAI Scenario Generator Tests (19 tests)")
    rc4 = run_cmd(
        "GenAI Scenario Generator Tests",
        f'"{sys.executable}" -m pytest personalized_precision_oncology/stage5_genai/genai/tests -v'
    )

    # 5. Run Evaluation Engineer Tests
    print_banner("5. Running Stage 5 Evaluation Tests (22 tests)")
    rc5 = run_cmd(
        "Evaluation Tests",
        f'"{sys.executable}" -m pytest personalized_precision_oncology/stage5_genai/evaluation/tests -v'
    )

    # 6. Run Integration Engineer Tests
    print_banner("6. Running Stage 5 Integration Tests (26 tests)")
    rc6 = run_cmd(
        "Integration Tests",
        f'"{sys.executable}" -m pytest personalized_precision_oncology/stage5_genai/integration/tests -v'
    )
    ...
    modules = [
        ...
        ("GenAI Scenario Generator Tests (19)", rc4),
        ("Evaluation Tests (22)", rc5),
        ("Integration Tests (26)", rc6)
    ]
    ...
    print("-" * 70)
    if all_passed:
        print("ALL 113 TESTS AND PIPELINES ARE FULLY FUNCTIONAL AND PASSING (100% SUCCESS)!")
    else:
