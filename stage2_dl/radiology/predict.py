...
class RadiologyPredictor:
    """
    Inference helper for Radiology CNN (CT/MRI slice analysis).
    """
    def __init__(self, checkpoint_path=None):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.preprocessor = ImagePreprocessor(target_size=(128, 128))
        self.model = RadiologyCNN(num_classes=2).to(self.device)
        self.classes = ['nodule_lesion', 'normal_tissue']
        
        if checkpoint_path is None:
            base_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
            checkpoint_path = os.path.join(base_dir, 'stage2_dl', 'artifacts', 'models', 'best_radiology_model.pth')
    ...
    def predict_image(self, img_path):
        """
        Predicts class probabilities for a single radiological CT/MRI scan slice.
        """
